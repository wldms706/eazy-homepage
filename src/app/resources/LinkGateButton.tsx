...
type Status = "idle" | "checking" | "loginRequired" | "profileRequired" | "ready" | "redirecting";

export function LinkGateButton({ url, label, buttonText, buttonStyle, resourceLabel }: LinkGateButtonProps) {
  ...
  return (
    <>
      <button onClick={openModal} className="resource-btn" style={{ ...buttonStyle, border: "none", cursor: "pointer", fontFamily: "inherit" }}>
        {buttonText}
      </button>
        ...
        >
          <div className="modal">
            ...
            {status === "loginRequired" && (
              <>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 16 }}>
                  <button type="button" onClick={() => handleLogin("kakao")} disabled={socialLoading} style={{ padding: "14px", background: "#FEE500", color: "#000", border: "none", borderRadius: 12, fontWeight: 700, fontSize: "0.95rem", cursor: "pointer", fontFamily: "inherit" }}>
                    {socialLoading ? "로그인 중..." : "카카오로 시작하기"}
                  </button>
                  <button type="button" onClick={() => handleLogin("google")} disabled={socialLoading} style={{ padding: "14px", background: "#fff", color: "#000", border: "1.5px solid #EBEBEB", borderRadius: 12, fontWeight: 700, fontSize: "0.95rem", cursor: "pointer", fontFamily: "inherit" }}>
                    Google로 시작하기
                  </button>
                </div>
            ...
            {status === "profileRequired" && (
              <>
                <h3>마지막 단계!</h3>
                <p>한 번만 입력하시면<br />다음부터는 바로 이용할 수 있어요.</p>
                <form onSubmit={handleSaveProfile}>
                  <input type="text" placeholder="원장님 성함" required minLength={2} value={name} onChange={(e) => setName(e.target.value)} />
                  <input type="tel" placeholder="연락처 (01012345678)" required pattern="[0-9\-]{11,13}" minLength={11} value={phone} onChange={(e) => setPhone(e.target.value)} />
                  <input type="text" placeholder="샵 상호명" required minLength={2} value={shop} onChange={(e) => setShop(e.target.value)} />
                  {error && <p style={{ fontSize: "0.85rem", color: "#FF4444", marginTop: 4 }}>{error}</p>}
                  <button type="submit" className="modal-submit">저장하고 이용하기</button>
                </form>
            ...
            {status === "redirecting" && (
              <div style={{ padding: 20, textAlign: "center" }}>
                <p style={{ fontSize: "1rem", fontWeight: 700, marginBottom: 4 }}>새 창으로 이동합니다!</p>
